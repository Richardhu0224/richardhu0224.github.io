...
Dear prof, the original chunk is not working. So i use ChatGPT to help me with the download of dataset. Hope you don't mind.
...
# 嘗試安裝 kingCoData（GitHub）
load_kingcodata <- function() {
  if (!requireNamespace("kingCoData", quietly = TRUE)) {
    if (!requireNamespace("devtools", quietly = TRUE)) install.packages("devtools")
    ok <- FALSE
    try({devtools::install_github("andykrause/kingCoData"); ok <- TRUE}, silent = TRUE)
    if (!ok) {
      if (!requireNamespace("remotes", quietly = TRUE)) install.packages("remotes")
      try({remotes::install_github("andykrause/kingCoData"); ok <- TRUE}, silent = TRUE)
    }
    if (!ok) return(FALSE)
  }
  library(kingCoData)
...
got_pkg <- load_kingcodata()

# 載入資料：優先用 package，失敗就用 CSV 備案
if (isTRUE(got_pkg)) {
  # 從資料包載入
  data("kingco_sales", package = "kingCoData", envir = environment())
} 

if (!exists("kingco_sales")) {
  # 備案：直接讀 GitHub 的原始 CSV（與套件內同源）
  # 若校園網路擋 GitHub，這步可能也需要 VPN。
  sales_url <- "https://raw.githubusercontent.com/andykrause/kingCoData/main/data/kingco_sales.csv"
  kingco_sales <- readr::read_csv(sales_url, show_col_types = FALSE)
}

# 檢查載入結果
stopifnot(exists("kingco_sales"))
```
